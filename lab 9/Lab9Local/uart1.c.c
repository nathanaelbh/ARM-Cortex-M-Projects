...
void uartPutChar2(char ch)
{
  UARTCharPut2(UART0_BASE, ch);
}

// Receive a character through UART0
char uartGetChar2()
{
	char ch = UARTCharGet2(UART0_BASE);
	return ch;
}

int uartPutString2(char *buffer)
{
	char ch;
	int n = 0;
...
int uprintf2(char* fmt, ...)
{
	...
	va_start2(argptr, fmt);
	vsnprintf2(buffer, LINE_SIZE+1, fmt, argptr);
	va_end2(argptr);

	// Send the formatted string to UART
	return uartPutString2(buffer);
}
